...
- [SixSentences](https://github.com/SixSentences/sixsentences) - self-hostable research workspace whose relevance to RR is the provenance chain it keeps intact across a project: a boolean literature search is compiled to SQL and run against a versioned local Parquet snapshot of the corpus, so the same query re-run later returns the same record set rather than whatever a live API answers today; screening decisions, data transformations, analyses and manuscript claims each stay linked to the source and the step that produced them. Apache-2.0, `docker compose` self-host
